...
One MySQL database, 15 tables, no ORM — every query in `backend/src/routes/*.js` is plain SQL via `mysql2/promise`. The schema lives across `backend/migrations/*.sql` in filename order (`001_init.sql` plus whatever's been added since — see §6's note on the `schema_migrations` tracking table, itself the 15th table, that now records what's been applied); this section explains the *reasoning* behind the shape of it, which the migration files themselves don't always spell out.
