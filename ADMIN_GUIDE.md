...
- **Clearing the log doesn't erase the fact that it was cleared** — both the individual-delete and clear-all routes write one fresh `activity_log` row immediately afterward, documenting who cleared what and when. That row is, deliberately, the one thing that survives a full wipe.

---

## 6. A few operational gotchas worth knowing

- **The database user has its own connection cap (`max_user_connections`), separate from the app's connection pool.** The live Clever Cloud database currently caps this specific database user at 5 concurrent connections. The app's own pool (`connectionLimit: 10` in `src/db.js`) rarely gets close to that under real traffic — but if you're running several one-off scripts against the live database back-to-back (each one opens its own short-lived connection pool, per this guide's own examples), you can hit `ER_USER_LIMIT_REACHED` fairly easily. If that happens: wait a few seconds for old connections to close, and pass `connectionLimit: 1` to any one-off script's `mysql.createPool(...)` call so it only ever holds a single connection.
