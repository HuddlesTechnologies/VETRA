...
| **Clever Cloud** | Hosts the MySQL database every piece of real data lives in. Free "Dev" plan. | console.clever-cloud.com → the MySQL add-on attached to this app. |
...
- **The database user has its own connection cap (`max_user_connections`), separate from the app's connection pool — and the app's pool is currently sized larger than that cap.** The live Clever Cloud database caps this specific database user at 5 concurrent connections, but `src/db.js`'s pool is configured with `connectionLimit: 10`. Under normal load the app hasn't needed more than a handful of simultaneous connections, so this hasn't caused a real outage yet — but it's a real, structural risk: if traffic ever needs 6+ simultaneous DB connections at once, the pool will try to open more than the account allows and some requests will fail with `ER_USER_LIMIT_REACHED`. This has only been triggered so far by running one-off scripts against the live database back-to-back (each opens its own short-lived pool, per this guide's own examples) — if that happens, wait a few seconds for old connections to close, and pass `connectionLimit: 1` to any one-off script's `mysql.createPool(...)` call. The real fix, when it's worth doing, is either lowering `connectionLimit` in `src/db.js` to something at or below 5, or upgrading the Clever Cloud plan to raise `max_user_connections`.
...
- **No rate limiting on auth endpoints.** Signin, admin-signin, signup, and password-reset-request have no brute-force/credential-stuffing protection (no `express-rate-limit` or equivalent anywhere in this codebase, confirmed by grep). Worth adding (e.g. a per-IP limit on failed signin attempts) before this app has enough real users to be worth attacking.
