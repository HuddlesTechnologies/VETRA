...
| Admin | `src/routes/admin.routes.js` | Customer/vendor management (Super Admin/Moderator only) with real order/revenue aggregates and per-account order history/activity/KYC for the detail pages, stats, role-scoped activity log, admin team + invite/verify/cancel (Super Admin only, real emails via Resend), vendor KYC review (Super Admin/Moderator only), platform settings (`GET`/`PATCH /settings` — guest checkout only so far) |
...
- **Cloudinary credentials are live on Render** — file uploads (avatars, cover photos, KYC documents, site banners, product images) work end-to-end in production.
...
- **Four of `admin/settings.html`'s five Platform Controls toggles** (vendor-approval, vendor-verification, auto-flag, maintenance mode) — still inert UI. Guest checkout is the one that's real (`platform_settings` table, `GET`/`PATCH /api/admin/settings`).
