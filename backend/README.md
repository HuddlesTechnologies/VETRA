...
There's no automated test suite yet — what's been manually verified so far is: `node --check` passes on every file in `src/`; the server boots and `GET /api/health` returns `{"ok":true}`; hitting an auth-required route with no token returns `401` instead of crashing; and hitting a DB-dependent route without a real database configured fails with a graceful `500` rather than taking the process down. Beyond that baseline, the full stack has been exercised end-to-end against the real live deployment (`https://vetra-api-11an.onrender.com`), its real Clever Cloud database, and the real Vercel-hosted frontend — not just locally, and not just via curl: a vendor signing up, listing a product (with a real image upload once Cloudinary is configured — see below), a buyer signing up, browsing the real catalog, adding to cart, and checking out; the vendor seeing that real order, updating its shipment status, and the buyer seeing that update reflected in their own order tracking; the buyer filing a report against the order and the vendor seeing it and submitting evidence. All of that was driven through the actual browser UI (not just the API directly), and all test accounts/orders/reports were cleaned out of the live database afterward. That's still manual verification, not automated test coverage — a real test suite (even a thin one hitting the routes above with `supertest` or similar) is worth adding before this goes anywhere near production traffic.
...
- **Cloudinary isn't configured on the live Render deploy yet** — `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` are declared in `render.yaml` as `sync: false` placeholders the dashboard has to be filled in by hand, and that hasn't happened. Until it does, `POST /api/uploads` 500s on every real request — meaning product image/video upload, vendor KYC document upload, and avatar/cover photo upload all fail in production even though the route code itself works (confirmed locally and by seeding products with plain URLs instead of real uploads). Sign up free at cloudinary.com (no card), copy the three values from its dashboard, paste them into Render → the `vetra-api` service → Environment, then redeploy.
...
- **Checkout double-submission** — `customer/assets/cart.js`'s checkout button disables itself immediately on click, but a real order is only rolled back by the *server's* own stock check failing; if the network stalls right after a successful write but before the client sees the response, a retry (by the shopper, or by a flaky connection) can create a second real order. Worth an idempotency key (a client-generated UUID sent with the request, deduplicated server-side) before this handles real payment traffic — found during end-to-end testing this round, not yet fixed.
