...
- **Admin's side of the review already has the shape this needs**: `admin/assets/data.js`'s mock `vendor.kyc` object (`status`, `cacNumber`, `idDocumentName`/`idDocumentUrl`, `cacDocumentName`/`cacDocumentUrl`, `submittedAt`, `reviewedAt`, `rejectionReason`) is effectively this table already, one row per vendor — `admin/vendor-detail.html`'s Verify/Reject buttons (`admin/assets/vendor-detail.js`) are the UI a real `PATCH /api/admin/vendors/:id/kyc` (§5) slots directly under.
...
6. **Admin roles: what each of the three `admin_role` values should actually be authorized to do.** The mock (`admin/assets/data.js`) has always had three named roles — Super Admin, Moderator, Support — and the real backend already enforces *some* of the distinction (`requireAdminRole("Super Admin")` gates the three admin-team routes in §5, and `GET /api/admin/activity` scopes non-Super-Admins to their own actions), but nothing today distinguishes Moderator from Support: both currently pass the same `requireRole("admin")` check as any other admin action. That's a real gap, not a deliberate design choice — the table below is the intended permission matrix a real build should enforce with a `requireAdminRole([...])` allow-list per route (extending the existing single-role version of that middleware to accept an array), not just document.
   ...
   | Change platform-wide settings (guest checkout, maintenance mode, vendor-approval/KYC-requirement toggles, site banners) | ✅ | ❌ | ❌ |
   | Reset demo data | ✅ | ❌ | ❌ |
   ...
   - **Moderator is the day-to-day trust & safety role** — every action that's a judgment call about a specific customer/vendor/report/KYC submission, but nothing that reconfigures the platform itself or touches who else has admin access. This is deliberately the same boundary the mock's Team & Roles copy already implies (`admin/settings.html`) without the backend having enforced it yet.
   ...
   - Enforce this **server-side only**. The current frontend doesn't hide or disable any of these actions differently per role at all (a Support admin sees the exact same buttons as a Super Admin) — that's a real UI gap worth fixing alongside the backend work (hide/disable what a role can't do, the same way `admin/activity.html`'s `?adminId=` filter is already Super-Admin-only in the UI), but the enforcement that actually matters is the `requireAdminRole([...])` check on the route, not what the client chooses to render.
...
**Frontend rewiring** (not in the original nine steps): done for `customer/` in full, for `vendor/`'s auth/products/orders, and for the entire `admin/` app (all 8 pages) — see the "Frontend wiring status" paragraph above §6 for exactly what's left (`vendor/profile.html`'s KYC/payout/avatar). Each remaining page is independently testable and doesn't depend on the others.
